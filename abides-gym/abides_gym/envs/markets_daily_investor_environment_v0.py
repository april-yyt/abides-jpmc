...
class SubGymMarketsDailyInvestorEnv_v0(AbidesGymMarketsEnv):
	...
	Initialized metrics by hyh
        """

        remaining_holding: float = 0  # at the end of the episode
        remaining_cash: float = 0 # at the end of the episode

        action_counter: Dict[str, int] = field(default_factory=dict)
        padded_return: float = 0
        holdings: float = 0
        imbalance: float = 0
        spread: int = 0
    ...
    def __init__(
        ...
        # performance tracker by hyh
        self.custom_metrics_tracker = (self.CustomMetricsTracker())
        self.custom_metrics_tracker.marked_to_market.append(starting_cash)
    ...
    def _map_action_space_to_ABIDES_SIMULATOR_SPACE(
        ...
        self.custom_metrics_tracker.action_counter[f"action_{action}"] += 1  # increase counter
        if action == 0:
            return [{"type": "MKT", "direction": "BUY", "size": self.order_fixed_size}]
        elif action == 1:
            return []
	...
	# log custom metrics to tracker
        self.custom_metrics_tracker.holdings = holdings[-1]
        self.custom_metrics_tracker.imbalance = imbalances[-1]
    ...
    def raw_state_to_reward(self, raw_state: Dict[str, Any]) -> float:
	    ...
	    # log custom metrics to tracker
            self.custom_metrics_tracker.marked_to_market.append(marked_to_market)
            self.custom_metrics_tracker.dense_reward = reward
    ...
    def raw_state_to_update_reward(self, raw_state: Dict[str, Any]) -> float:
        ...
        if self.reward_mode == "dense":
            ...
            self.custom_metrics_tracker.marked_to_market.append(marked_to_market)
            self.custom_metrics_tracker.sparse_reward = reward
    ...
    def raw_state_to_done(self, raw_state: Dict[str, Any]) -> bool:
        ...
        self.custom_metrics_tracker.remaining_cash = cash
        self.custom_metrics_tracker.remaining_holding = holdings
